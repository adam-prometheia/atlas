...
- **LLM access:** `app/llm.py` wraps the OpenAI Python SDK. `_invoke_model` prefers the Responses API with a chat-completions fallback and applies a neutral system message.
...
- **Draft Custom Email:** Opens `/contacts/{id}/draft_custom_email` with purpose/tone selectors, required brief, optional context, live greeting preview, and website snapshot accordion. Generated drafts can be copied to the clipboard with one click.
